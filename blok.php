...
(SELECT casting.NOBUKTI, casting.TANGGAL, det_casting.PV_ID, PV_NAMA, det_casting.KUBIK, 'CASTING' AS tipe FROM casting, det_casting WHERE casting.NOBUKTI=det_casting.NOBUKTI AND MONTH(casting.TANGGAL)=MONTH(NOW()) AND YEAR(casting.TANGGAL)=YEAR(NOW()) 
UNION
...
SELECT apm.NOBUKTI, apm.TANGGAL, PV_ID, PV_NAMA, det_apm.KUBIK, 'STOK' AS tipe FROM apm, det_apm WHERE apm.NOBUKTI=det_apm.NOBUKTI AND (apm.NOBUKTI LIKE 'TB%' OR apm.NOBUKTI LIKE 'TM%' or apm.NOBUKTI like 'TW%') AND MONTH(apm.TANGGAL)=MONTH(NOW()) AND YEAR(apm.TANGGAL)=YEAR(NOW())
UNION
SELECT apk.NOBUKTI, apk.TANGGAL, PV_ID, PV_NAMA, det_apk.KUBIK*-1, 'STOK' AS tipe FROM apk, det_apk WHERE apk.NOBUKTI=det_apk.NOBUKTI AND MONTH(apk.TANGGAL)=MONTH(NOW()) AND YEAR(apk.TANGGAL)=YEAR(NOW())
UNION       
SELECT suratjalan.SJ_NO, suratjalan.TGL_SJ AS TANGGAL, PV_ID, PV_NAMA, det_suratjalan.KUBIK*-1, 'STOK' AS tipe FROM suratjalan, det_suratjalan WHERE suratjalan.SJ_NO=det_suratjalan.SJ_NO AND MONTH(suratjalan.TGL_SJ)=MONTH(NOW()) AND YEAR(suratjalan.TGL_SJ)=YEAR(NOW())
...
)e WHERE PRODUK='BLOK' GROUP BY TANGGAL, PRODUK;";
*/	
	$sql = "SELECT TANGGAL, PRODUK, SUM(kolcasting) as GETCASTING, SUM(kolntbj) as GETNTBJ, SUM(kolsj) as GETSJ, SUM(kolstok) as GETSTOK from
(
SELECT TANGGAL, PRODUK, IF(tipe='CASTING',PROGRESS,0) as kolcasting, IF(tipe='NTBJ',PROGRESS,0) as kolntbj, IF(tipe='SJ',PROGRESS,0) as kolsj, IF(tipe='STOK',PROGRESS,0) as kolstok from
